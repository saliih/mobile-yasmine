...
(function (root, factory) {
    ...
    var NumberControl = Backform.NumberControl = Control.extend({
        ...
        template: _.template([
            '<label class="<%=Backform.controlLabelClassName%>"><%=label%></label>',
            '<div class="<%=Backform.controlsClassName%>">',
            '  <input  <% if(typeof(title)!="undefined"){%> title="<%=title%>"<% } %>  type="<%=type%>" max="<% if (typeof(max) == "function") max = max(this.model);%><%=max%>" min="<%=min%>" class="<%=Backform.controlClassName%> <%=extraClasses.join(\' \')%>" name="<%=name%>" value="<%-value%>" placeholder="<%-placeholder%>" <%=disabled ? "disabled" : ""%> <%=required ? "required" : ""%> />',
            '  <% if (helpMessage && helpMessage.length) { %>',
            '    <span class="<%=Backform.helpMessageClassName%>"><%=helpMessage%></span>',
            '  <% } %>',
            '</div>'
